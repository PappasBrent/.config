...
# If SSH'd into a machine, print the username and hostname.
ps1_prefix=""
if [[ -n "$SSH_TTY" ]]; then
    ps1_prefix="\u@\h:"
fi

# Also display current git branch in prompt
if command -v git >/dev/null 2>&1; then
    export PS1="${ps1_prefix}\[\e[32m\]\w \[\e[91m\]\$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/(\1)/')\[\e[00m\]$ "
else
    export PS1="${ps1_prefix}"
fi
...
if command -v gsettings >/dev/null 2>&1 \
    && gsettings get org.gnome.mutter center-new-windows >/dev/null 2>&1; then
